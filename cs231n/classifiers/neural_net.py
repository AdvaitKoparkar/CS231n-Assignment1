...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    ## First Hidden Layer
    o1 = np.dot(X, W1) + b1[:, None].T
    X_app = np.hstack((X, np.ones((N, 1))))
    W1_app = np.vstack((W1, b1[None, :]))
    o1 = np.dot(X_app, W1_app)
    # ReLU
    o1_d = o1
    o1_d[o1_d <= 0] = 0
    ...
    # Second Hidden Layer
    # scores = np.dot(o1, W2) + b2[:, None].T
    o1_app = np.hstack((o1_d, np.ones((N, 1))))
    W2_app = np.vstack((W2, b2[None, :]))
    scores = np.dot(o1_app, W2_app)
    ...
    # classifier loss.                                                          #
    #############################################################################
    # scores -= np.max(scores,axis=1,keepdims=True) ## why?
    exp_scores = np.exp(scores)
    sum_exp_scores = np.sum(exp_scores, axis=1)
    loss_mat = -1*scores[np.arange(N), y] + np.log(sum_exp_scores)
    loss = np.mean(loss_mat)
    loss += 0.5*reg*(np.sum(W1*W1) + np.sum(W2*W2)) # + np.sum(b1**2) + np.sum(b2**2))
    ...
    dW2_app = np.dot(o1_app.T, dscores) / N

    do1_app = np.dot(dscores, W2_app.T)

    o1_indicator = o1_d
    o1_indicator[o1_indicator > 0] = 1
    do1 =  o1_indicator*do1_app[:, 0:H]

    dW1_app = np.dot(X_app.T, do1) / N

    dW1 = dW1_app[0:D, :] + reg*W1
    db1 = dW1_app[D, :]

    dW2 = dW2_app[0:H, :] + reg*W2
    db2 = dW2_app[H, :]
